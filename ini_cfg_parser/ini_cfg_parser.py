...
class IniParser:
    ...
    def _get_value(self, section: str, key: str, fallback) -> Any:
        ...
        try:
            return self.ini_dict[section][key]['inf']
        except KeyError:
    ...
    def read_inifile(filename: str, config: configparser.ConfigParser) -> None:
        ...
        try:
            ...
            if not files_read:
                raise FileNotFoundError('INI file could not be read')
    ...
    # @retval       res_flag        : result flag [type bool]
    # @retval       opt_re          : result value [type Union[int, re.RegexFlag]]
    # @retval       msg             : result message [type str]
    @staticmethod
    def re_option_check(str_opt: str) -> Tuple[bool, Union[int, re.RegexFlag], str]:
